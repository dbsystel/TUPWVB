...
TUPWVB should only be used if a key management system is not available.

The idea of TUPWVB is to store secrets in an encrypted form and to distribute the elements the key is derived from, so it is not found in a single place.
...
If one use OpenShift, Kubernetes, GitLab or any other software that puts secrets in environment variables, files or imagePullSecrets, these secrets can also be encrypted with TUPWVB.
...
TUPWVB helps here in that the secret is nowhere stored in clear text format.
...
The above mentioned keys are derived from this HMAC of the data that is supplied to the classes used by TUPWVB.
...
TUPWVB is released under the Apache license, V2.0. See "LICENSE" for details.
